...
// Iterate 0 to n removing character position n in str and pass that to callback
// (lambda). This is knowingly very inefficient. Some of the use-case is
// builtin, i.e. iterating until lambda returns true. It is not really very
// efficient, in that it is only generating n cases, but it does cost the heap
// allocation.
{
...
bool is_palindrome_except_one_slow(const char *str, size_t n) {
  ...
  return generate_except_one(
      str, n, [](const char *str, size_t n) { return is_palindrome(str, n); });
}
...
#define MAKE_STR(s) {(s), (sizeof(s) - 1)}
  ...
  for (auto s : data) {
    ...
    generate_except_one(s.chars, s.size, [&](const char *str, size_t n) {
